...
require_once("functions/calculos.php");
	...
	$cont = (int) 0;
	$mostrarQuantidadePar = (int) 0;
	$mostrarQuantidadeImpar = (int) 0;
        ...
        if($valorIncial > $valorFinal){
			...
			for($cont = $valorIncial ; $cont <= $valorFinal ; $cont++){
				...
				if($resultado == 0){
				   ...
				   $mostrarQuantidadePar = $mostrarQuantidadePar + 1 ;
					
				} else {
					$resultadoImpar = $resultadoImpar . $cont . "<br>";
					$mostrarQuantidadeImpar = $mostrarQuantidadeImpar + 1 ;
		...
		<form  name="frmImparPar" method="post" action="">
			<div id="caixaInserir">
                    ...
                    <select class="estiloInput" name="sltInicial"> 
                       <option value=""> Insira um Número </option>
                        <?php 
                            $cont = (int) 0;
            ...
            <div id="caixaPar" > 
				<p> Números Pares </p>
                <div id="numeroPar"> <?php
                   echo($resultadoPar);
                    ?>
                </div>
                <p> Quantidade de Impar: <?=$mostrarQuantidadePar?> </p>
            </div>
			
			
            <div id="caixaImpar"> 
				<p> Numeros Impares </p>
                <div id="numeroImpar" >  <?php echo($resultadoImpar); ?> </div>
                <p> Quantidade de Par: <?=$mostrarQuantidadeImpar?> </p>
            </div>
